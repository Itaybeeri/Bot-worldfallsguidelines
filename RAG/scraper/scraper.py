...
import time
try:
    # When run as a package (python -m scraper.scraper)
    from scraper.config import WEBSITE_URL, DATA_DIR as CONFIG_DATA_DIR, PDF_DIR as CONFIG_PDF_DIR, HTML_DIR as CONFIG_HTML_DIR
except Exception:
    # Fallback when running the script directly (python scraper/scraper.py)
    # Load the `config.py` file from the same folder via importlib so imports
    # work whether the package is executed as a module or the script is run
    # directly.
    import importlib.util
    cfg_path = os.path.join(os.path.dirname(__file__), "config.py")
    spec = importlib.util.spec_from_file_location("scraper_config", cfg_path)
    cfg = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(cfg)
    WEBSITE_URL = cfg.WEBSITE_URL
    CONFIG_DATA_DIR = cfg.DATA_DIR
    CONFIG_PDF_DIR = cfg.PDF_DIR
    CONFIG_HTML_DIR = cfg.HTML_DIR
